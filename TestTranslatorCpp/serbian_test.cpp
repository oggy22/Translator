...
namespace TranslatorTest
{
	TEST_CLASS(SerbianTests)
	{
		...
		{
			Assert::AreEqual(expected, translator::parse<Serbian>(st) == 1.0, st.c_str());
		}
...
#define CHECK check<Serbian>
		...
		TEST_METHOD(check_some_noun_forms)
		{
			...
			CHECK(L"гусака", L"гуска", attr_t::множина, attr_t::генитив);
			CHECK(L"дасака", L"даска", attr_t::множина, attr_t::генитив);
			CHECK(L"дрвету", L"дрво", attr_t::једнина, attr_t::датив);
			CHECK(L"ђаци", L"ђак", attr_t::множина, attr_t::номинатив);
			...
			//CHECK(L"песама", L"песма", attr_t::множина, attr_t::генитив);
		...
		TEST_METHOD(check_some_verb_forms)
		{
			...
			CHECK(L"машем", L"махати", attr_t::једнина, attr_t::лице1);
			CHECK(L"машу", L"махати", attr_t::множина, attr_t::лице3);
...
#define NEXISTS(word) Assert::IsFalse(dictionary_word_exists(word), word)
		...
		TEST_METHOD(dictionary_words_count)
		{
			...
			Assert::AreEqual<size_t>(1592, Serbian::dictWords().size());
		}
		...
		TEST_METHOD(word_forms_count)
		{
			...
			{
				size_t increment = dw.words.size();
				Assert::IsFalse(increment == 0);
				count += increment;
			}

			// Update this number when necessary
			Assert::AreEqual<size_t>(36318, count);
		}
		...
		{
			for (int i = 0; i < DebugRelease(20, 180); i++)
			{
				// This one happens to be really long
				if (i == 17)
					continue;
